...
본 프로젝트는 신용카드 거래 데이터의 극심한 불균형 문제를 해결하기 위해, 논문에서 제안된 **K-CGAN(K-Means Conditional GAN)** 기법을 구현하고, 이를 최신 정형 데이터 특화 딥러닝 모델들과 결합하여 탐지 성능(F1-Score, Recall)을 극대화하는 것을 목표로 합니다.
...
분석가로서 데이터 증강 효율을 검증하기 위해 **4 x 4 실험 매트릭스**를 구성하여 총 16번의 독립적인 실험을 수행합니다.

### 3.1 실험 매트릭스
| 분류 모델 \ 데이터셋 | Original | SMOTE | cGAN | **K-CGAN** |
| :--- | :---: | :---: | :---: | :---: |
| **Random Forest** | Exp 1 | Exp 5 | Exp 9 | Exp 13 |
| **TabNet** | Exp 2 | Exp 6 | Exp 10 | Exp 14 |
| **BERT (Tabular)** | Exp 3 | Exp 7 | Exp 11 | Exp 15 |
| **AutoEncoder** | Exp 4 | Exp 8 | Exp 12 | Exp 16 |
...
```text
├── notebooks/
│   ├── 01_data_eda_preprocessing.ipynb    # 데이터 분포 확인 및 스케일링
│   ├── 02_clustering_k_means.ipynb       # 사기 데이터 군집 분석
│   ├── 03_gan_model_training.ipynb       # cGAN 및 K-CGAN 구현 및 학습
│   └── 04_benchmark_experiments.ipynb    # 16회 실험 및 결과 시각화
├── models/
│   ├── saved_gan_weights/                # 생성 모델 가중치
│   └── saved_classifier_weights/         # 최적 분류 모델 가중치
├── results/
